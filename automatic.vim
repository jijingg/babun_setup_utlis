...
if exists("s:autoinst_prefix_max_len") == 0
    let s:autoinst_prefix_max_len = 26
endif
if exists("s:autoinst_suffix_max_len") == 0
    let s:autoinst_suffix_max_len = 30
endif
...
let s:VlogTypeData = s:VlogTypeData . '\<signed\>\|'
...
map <S-F1>                                                              :call AutoInst(0)<ESC>
map <S-F2>                                                              :call AutoArg()<ESC>
map <S-F3>                                                              :call AutoDef()<ESC>
...
function AutoTemplate() "{{{2
    ...
    if modulename == 'top'
        ...
        call append(29, "    rstn = 1'b0;")
        call append(30, "    #52 rstn = 1'b1;")
        call append(31, "end")
...
function Ifsdb() "{{{2
    ...
    call setline(lnum+1,"initial begin")
    call setline(lnum+2,"    $fsdbDumpfile(\"./hw.fsdb\") ;")
    call setline(lnum+3,"    $fsdbDumpvars(0,xxtb);")
    call setline(lnum+4,"    $fsdbDumpflush;")
    call setline(lnum+5,"end")
    call cursor(lnum+3,22)
endfunction "}}}2
function AlBpn() "{{{2
    let lnum = line(".")
    ...
    call setline(lnum+1,"always @(posedge clk or negedge rstn)")
    call setline(lnum+2,"    if(!rstn) ")
    call setline(lnum+3,"        ")
    call setline(lnum+4,"    else if() ")
    call setline(lnum+5,"    else ")
...
function AlBnn() "{{{2
    ...
    call setline(lnum+1,"always @(negedge clk or negedge rstn) begin")
    call setline(lnum+2,"    if(!rstn) begin")
    call setline(lnum+3,"        ")
...
function WireDef2PortConn() "{{{2
    ...
    if line =~ '('
        ...
        let suffix_margin = s:CalMargin(s:autoinst_suffix_max_len, len(sig)+len(width)+4)
...
function s:GetInstFileName(inst) "{{{2
    let inst_file = ''
    let file_readable = 0
    if filereadable('vtags')
        let lines = readfile('vtags')
        for line in lines
            if line =~ '^' . '\<' . a:inst . '\>'
...
"       [width,type,has_defined,seq,line,signal_name,io_dir,last_port]
                    ...
                    if type =~ 'keep'  " jijing@20180320 remove comment and null line in ports
                        " if value[4] =~ '^\s*\/\/'
...
function AutoFsm() "{{{2
    ...
    for line in getline(1, line("$"))
        if line =~ '/\*\<autofsm\>'
            ...
            call add(aft_fsm, "always @(posedge clk or negedge rstn) begin")
            call add(aft_fsm, s:indent . "if(!rstn) begin")
...
function s:oTreeNode.New(parent,filename,instname,instname2) dict
